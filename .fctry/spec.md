...
> **Target status:** The lifecycle verbs below (`pull`, `install`, `uninstall`, `remove`, plus the `library`, `browse`, `snapshots`, `rollback`, and type-specific `agents` / `commands` / `hooks` / `settings` subcommands) are the v2.0.1 CLI surface. They are not yet built — `src/cli/index.ts` still ships the v1.3 verbs. The rewrite lands with the v2.0.1 target modules (see §Architecture → Modules (v2.0.1 targets)) and the coordinated migration plan (see §Migration).
...
> **Target status:** The sub-router inventory below includes routers that ride with the v2.0.1 target modules — `agents`, `commands`, `hooks`, `settings`, `snapshots`, and `browse` — see §Architecture → Modules (v2.0.1 targets). Today's `appRouter` ships the subset tied to built modules; the target sub-routers land when their backing modules do.
...
The modules below are described in the spec as v2.0.1 targets. They do not exist on disk yet. This sub-section is the **canonical home** for the target-vs-built distinction — `CLAUDE.md`'s prior "Target modules (v2.0.1, not yet built)" block has been collapsed into a pointer to this section.
...
- **Unified installed-plus-discoverable fuzzy search (`fuzzySearchAll()`)** — Single search across installed resources and discoverable catalog content with `@marketplace-name` filter syntax parsed from the query. This would be the engine behind `ensemble browse`. (Pattern from plum.) Deferred — `ensemble browse` is itself a v2.0.1 target module, and this engine rides with it.
