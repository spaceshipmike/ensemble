...
| Module | Role (target) |
|--------|---------------|
| `browse.ts` | Library discovery engine — fuzzy search across installed + discoverable resources, `@marketplace-name` filter parsing; drives both the `ensemble browse` CLI (plain-text output) and the desktop Registry view. Pure-function primitive; no presentation code. |
